...
public class Green3 implements LevelInformation {
     ...
     */
    public Green3(int width, int height, int bordersThick) {
    ...
    public List<Velocity> initialBallVelocities() {
        ...
        velocities.add(Velocity.fromAngleAndSpeed(40, 400));
        velocities.add(Velocity.fromAngleAndSpeed(-40, 400));
        return velocities;
    }
    /**
    ...
    public Sprite getBackground() {
        return new Green3Background(this.getGameSpace());
    }
    ...
    public void setBlocks() {
        ...
        int obWidth = (int) (this.gameSpace.getWidth() / 15);
        ...
        Point upperLeft = new Point(x + this.getGameSpace().getWidth() - obWidth,
                y + 4.25 * obHeight);
        for (int i = 1, k = 10; i < 6; i++, k--) { //go over lines
            for (int j = 0; j < k; j++) { //one line of obstacles.
